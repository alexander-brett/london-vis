...
function makeOptions(database){
  ...
  var radiusMap = {
      ...
      "Median House Price (£)": radiusWithYears('median_house_price', 'price', 3.5, database),
    };
...
function startForce(data, db){
    ...
    d3.selectAll('input[name=colourOption],input.colour.year').on("change.main", function(){
      var colourGenerator = options.makeColour(db);      
      drawColourKey(svg, colourGenerator);
